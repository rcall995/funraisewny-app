  ...
  <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: index * 0.05 }} whileHover={{ scale: 1.03, y: -5 }} className="bg-white rounded-xl shadow-lg overflow-hidden cursor-pointer border border-transparent hover:border-blue-500 group" role="button" onClick={() => onQuickView(deal)}>
    <div className="p-5">
      <div className="flex items-center mb-4">
        {deal.logo_url ? <Image src={deal.logo_url} alt={`${deal.business_name} logo`} width={48} height={48} className="rounded-full mr-4 object-cover border-2 border-gray-100" /> : <div className="w-12 h-12 rounded-full bg-gray-100 mr-4 flex-shrink-0"/>}
        <div>
          <h3 className="font-bold text-gray-800 group-hover:text-blue-600 transition-colors">{deal.business_name || 'Local Business'}</h3>
          <div className="text-xs text-gray-500 flex items-center mt-1"><MapPin size={12} className="mr-1 flex-shrink-0" /><span>{deal.address}</span></div>
        </div>
      </div>
      <h2 className="text-lg font-bold mb-2 text-gray-900">{deal.title}</h2>
      <p className="text-gray-600 text-sm leading-relaxed h-10 overflow-hidden" style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{deal.description}</p>
    </div>
    {deal.terms && <div className="text-xs text-gray-500 italic px-5 py-3 border-t bg-gray-50/50">Terms: {deal.terms.length > 50 ? `${deal.terms.substring(0, 50)}...` : deal.terms}</div>}
  </motion.div>
);
      ...
      {isOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={onClose}>
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} transition={{ type: 'spring', stiffness: 400, damping: 30 }} className="bg-white rounded-lg p-8 max-w-lg w-full m-4 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={onClose} className="block w-full bg-blue-600 text-white py-3 px-4 rounded-lg text-center font-semibold hover:bg-blue-700 transition-colors"> Close </button>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
