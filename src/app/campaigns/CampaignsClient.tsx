...
import type { Campaign, Membership } from './page';
...
const ShareButton = ({ url, copiedId, setCopiedId, campaignId }: {
  url: string;
    ...
    try {
      ...
      const campaignsWithMemberships = await Promise.all(
        data.map(async (campaign) => {
          ...
          const membershipsWithProfiles: Membership[] = await Promise.all(
          ...
          return { ...campaign, memberships: membershipsWithProfiles };
        })
      );
