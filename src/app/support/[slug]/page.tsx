...
type Campaign = {
  ...
  // Note: Added slug here for safety, though it comes from params
  slug: string; 
};
...
export default function SupportPage() {
  ...
  const formatSupporterName = useCallback((fullName: string | null) => {
    ...
    const first = parts[0];
    // Use the last part of the name for the initial
    const lastInitial = parts.length > 1 ? parts[parts.length - 1].charAt(0) : '';
    
    // Check if we have at least a first name and an initial
      ...
      if(campaignData) {
        ...
        if (user) {
            ...
            if (profileData?.full_name) {
                ...
                // Find the last name part, handling single-word names gracefully
  ...
  return (
    // Added bg-gray-50 to shade the page background, making the white card stand out
    <div className="min-h-screen bg-gray-50 flex flex-col items-center p-4 md:p-8"> 
      <div className="w-full max-w-2xl text-center">
        ...
        {/* Main white card for membership/purchase info */}
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 space-y-2">
              {supporters.map((supporter, index) => (
                // Changed background to white for contrast against the page's bg-gray-50
